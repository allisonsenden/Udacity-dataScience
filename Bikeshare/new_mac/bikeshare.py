...
def get_city():
    ...
    while True:
        ...
        try:
            ...
            print('Oops! That was not a valid city input. Try again...')
        if city == 'chicago':
...
def popular_trip(city, time_period):
    ...
    '''
    # TODO: complete function
    return city['stationCombo'].value_counts().index[0].rstrip(', ')
...
def display_data(city):
    ...
    while True:
        display = input('\nWould you like to view individual trip data?'
                    ' Type \'yes\' or \'no\'.\n').lower()
        try:
            value = display.isalpha()
        except ValueError:
            print('Oops! That was not a valid response. Try again...')
        if display == 'yes':
            print(pprint.pprint(city[start:end]))
            start += 5
            end += 5
        if display == 'no':
            break
...
def statistics():
    ...
    city['month'] = city.startTime.dt.month
    city['year'] = city.startTime.dt.year
    city['day'] = city.startTime.dt.day
    city['hour'] = city.startTime.dt.hour
    city['yearsOld'] = city['year'] - city['birthYear']
    city['duration'] = (city['endTime'] - city['startTime']).astype('timedelta64[s]')
    city['stationCombo'] = city[['startStation', 'endStation']].apply(lambda c: ''.join(c + ', '), axis = 1)
